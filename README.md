...
3. **配置服务器信息**编辑 `config.py` 文件，添加服务器信息：
...
1. **访问前端界面**打开浏览器，访问：http://localhost:9002/static/index.html

2. **使用自然语言**在输入框中输入自然语言，例如：
   
   * "查看CPU使用率"
   * "ping一下百度官网"
   * "检查内存使用情况"

3. **使用Linux命令**切换到"Linux命令"模式，直接输入Linux命令，例如：
   
   * `ls -la`
   * `ps aux | grep python`
   * `df -h`

4. **使用快捷操作**点击左侧的快捷操作按钮，快速查看系统信息：
